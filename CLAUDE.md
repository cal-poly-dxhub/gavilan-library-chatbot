...
- `infra/` - CDK Python app. Provisions the S3 Vectors store (`CfnVectorBucket` + `CfnIndex`), the KB role, `AWS::Bedrock::KnowledgeBase` (S3_VECTORS storage) + `AWS::Bedrock::DataSource` type S3, the KB source bucket, the scraper Lambda (+ deps layer, one EventBridge rule per tier, one-click deploy Trigger), the dedicated catalog bucket, the query path (Lambda + own role, HTTP API with `POST /query` and `GET /warm`, both public and unauthenticated), the theme save path (a Cognito pool with managed login + hosted domain + branding, its own JWT authorizer on `PUT /theme`, and the key-scoped theme-save Lambda - see "Widget theme file"), and - when `feedback` is configured - the feedback path (SNS topic + email subscription + its own small Lambda/role/log group + `POST /feedback`). Also hosts the widget (private S3 + CloudFront OAC, three sources in the widget `BucketDeployment` plus a second one for `defaults/`) in the SAME stack (OAC cross-stack cyclical dependency), the demo site (a SECOND private S3 + CloudFront OAC pair, see below), and defines the Bedrock guardrail (one `CfnGuardrail` + one `CfnGuardrailVersion`). Outputs the paste-ready embed tag, the CloudFront domain, the `/query` URL, `KnowledgeBaseId`, `DemoSiteUrl`, and the theming trio (`WidgetThemeEditor`, `WidgetThemeUpload`, `ThemeAdminCreateUserCommand`).
  - `app.py` - CDK entrypoint; loads `config.yaml`, passes to stack
  - `infra/infra_stack.py` - the stack (`GavilanChatbotStack`)
  - `infra/config.py` - `load_config()`; resolves repo-root `config.yaml` from `__file__`. Also the synth-time validators: `resolve_cors_allow_origins()` (rejects `*`), `resolve_feedback()` (three outcomes: off / on-with-a-destination / a malformed address, which raises), and `resolve_scraper_tiers()` / `resolve_seed_urls()`
  - `tests/unit/` - `test_infra_stack.py` (Template.from_stack assertions), `test_handler.py` (boto3 stubbed, payload-2.0 events), `test_feedback_handler.py` and `test_theme_handler.py` (same stubbing, for the two small Lambdas)
- `app/` - query Lambda. `handler.py`: HTTP API (payload 2.0) entrypoint; `run_agent()` runs the agentic Converse tool-use loop over four tools (`search_library_info` -> KB `Retrieve`; `database_catalog` -> reads the catalog from S3; `search_book_catalog` + `search_course_reserves` -> live Primo API calls, client inline in `handler.py`, no import). `_links_block()` renders the bundled link table into the `system` payload. Wiring from env vars set by the stack. boto3 provided by the runtime.
  - `system_prompt.md` - finalized system prompt. `<tools>` carries the four-tool routing guidance, including that course textbooks and reserve materials go to `search_course_reserves` and not the general catalog; `<priority_responses>` is checked first and answers safety/emergency messages verbatim with no tool call. Read once at cold start, passed via Converse `system`. Packaged via `Code.from_asset(app/)`. NOTE: the curated link table is deliberately absent from `<tools>` because it is no longer a tool - `<citations>` governs it instead, naming the injected CANONICAL GAVILAN LINKS block as one of exactly two permitted link sources.
...
- `eval/` - Bedrock RAG eval harness (boto3 tooling, NOT CDK), on demand against deployed infra. A retrieve-only formatter (ContextRelevance / ContextCoverage) and a retrieve-and-generate one (answer quality, bring-your-own-inference); `capture_outputs.py` is a STUB - it defines the shape but does not call the deployed `/query` yet. Separate `eval_config.yaml`. Two things here DO run offline: `run_chunking_eval.py` (chunk boundaries over the corpus) and the unit tests.
  - `promptfoo/` - the answer-quality loop: 22 golden questions at the deployed `/query`, scored by a Bedrock judge on behavior and groundedness. Not a CI gate and must not become one (it spends real money per row). Own README.
...
- `frontend/` - embeddable widget + the demo page. FIVE files ship, to two SEPARATE buckets (four to the widget bucket, one to the demo bucket):
  ...
  - `demo-site.html` - the shareable demo page, uploaded as `index.html` to the demo bucket. A Gavilan-Library-styled sample page (local CSS only, nothing hotlinked from gavilan.edu) carrying the SAME one-line embed a library page would, so it cannot fork or drift from the shipped widget. Placeholders for the widget src, the `/query` URL and the `cost_model` block are stamped at DEPLOY time (`s3deploy.Source.data` resolves CDK tokens during deployment) - nothing in it is account- or region-specific. Renaming one fails synth.
