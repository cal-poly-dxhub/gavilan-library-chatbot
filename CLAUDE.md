...
- **Demo site:** one static page (`frontend/demo-site.html`) on its OWN S3 + CloudFront, embedding the production widget from the production CDN. `cdk deploy` stamps the live API + widget URLs into it and outputs `DemoSiteUrl`. Toggle with `demo_site.enabled` in config.yaml
...
- `frontend/` - embeddable widget + the demo page. TWO files ship, to two SEPARATE buckets:
  - `widget.js` - the production widget (production-clean, no mock code); reads its endpoint from its `<script>` tag's `data-api-url`, POSTs a multi-turn `{messages: [...]}` array, renders `{answer, sources}`. The ONLY file in the widget bucket.
