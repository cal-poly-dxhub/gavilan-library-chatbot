...
widget bucket root (its copy is authored in `docs/theming-quickstart.md` and rendered
verbatim). `docs/widget-theming.md` stays as the repo's developer guide.
...
- **`defaults/theme.json` is the download** - a deployment-owned copy of the built-in
  ...
  so the file must explain itself. The text is authored as the fenced block in
  `docs/theme-settings-reference.md` and shipped verbatim; the contract suite pins the byte
  equality AND the self-documentation. It replaced `theme.example.json`; the repo must
...
- **`theme-guide.html` is the hosted guide**, one self-contained page at the widget bucket
  root, its copy rendered verbatim from `docs/theming-quickstart.md`. No scripts, no
...
- `docs/` - design docs (`architecture.md`, `build-plan.md`, architecture diagram), the accessibility audit, `widget-theming.md` (the developer guide to `theme.json`), `theming-quickstart.md` (the authored copy of the hosted guide page) and `theme-settings-reference.md` (the authored text of `defaults/theme.json`, shipped verbatim, plus the claims log checked before shipping).
